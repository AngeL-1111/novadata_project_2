...
cursor.execute("""SELECT id, username, event_type, extract(epoch FROM event_time)
                  FROM user_logins
...
for row in rows:
    ...
    cursor.execute("UPDATE user_logins SET sent_to_kafka = TRUE WHERE id = %s", (row[0],)) # by id
    conn.commit()
